...
.then(data => {
    ...
    Object.entries(data.skills).forEach(([category, skills]) => {
        skillsContainer.innerHTML += `<h3 class="text-xl font-semibold text-gray-800 mb-2">${category.charAt(0).toUpperCase() + category.slice(1)}</h3>`;
        skillsContainer.innerHTML += `<ul class="grid md:grid-cols-4 grid-cols-2 gap-4 mb-6">${skills.map(skill => `<li class="bg-white rounded-lg p-2">${skill}</li>`).join('')}</ul>`;
    });
                ...
                <div>
                    <div class="text-gray-600">
                        ...
                        <div class="flex text-sm">
                            <a href="${project.links.source}" class="text-blue-600 underline pr-2">Source Code</a>
                            <p>|</p>
                            <a href="${project.links.visit}" class="text-blue-600 underline pl-2">Visit Project</a>
                        </div>
                    </div>
                </div>
            </div>`;
    ...
    data.achievements.forEach(achievement => {
        achievementsContainer.innerHTML += `<li class="bg-white p-3 rounded-xl">${achievement}</li>`;
    });
    ...
    data.hobbies.forEach(hobby => {
        hobbiesContainer.innerHTML += `
            <div class="bg-white rounded-lg p-4 mb-6">
                <div class="mb-4">
                    <h3 class="text-xl font-bold text-gray-800">${hobby.title}</h3>
                </div>
                <div>
                    <div class="text-gray-600">
                        <p class="text-gray-600 text-sm mb-4">${hobby.description}</p>
                    </div>
                </div>
            </div>`;
    });
    ...
    data.certifications.forEach(cert => {
        certificationsContainer.innerHTML += `<li class="bg-white p-3 rounded-xl">${cert.title} - <span class="bg-gray-100 font-bold p-2 text-sm rounded-xl">${cert.provider}</span></li>`;
    });
    ...
    data.footer.socialLinks.forEach(link => {
        footerSocialLinks.innerHTML += `
            <a href="${link.url}" target="_blank">
                <li class="m-2 p-2">
                    <div>
                        <img src="${link.icon}" class="w-7 h-7 invert" alt="${link.platform}">
                    </div>
                </li>
            </a>`;
    });
